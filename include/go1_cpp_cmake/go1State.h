...
class go1State {
    ...
    go1State();
    void resetState();
    void updateStateFromMujoco(const mjtNum* q_vec, const mjtNum* q_vel, const Eigen::Vector3d& lin_acc, bool init = false);
    void convertForcesToTorques(const mjtNum* q_vec);
    ...
    double foot_deltaX;
    double foot_deltaY;
    Eigen::Matrix<double, 3, NUM_LEG> root_hip_pos; // absolute frame
    ...
    Eigen::Matrix<double, 3, NUM_LEG> foot_pos_d; // absolute frame
