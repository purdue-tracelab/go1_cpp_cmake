...
void runTorque() {
    ...
    if (init_count >= 20) {
        ...
        for (int i = 0; i < NUM_LEG; i++) {
            tester_state.joint_torques(0, i) = ((i == 1 || i == 3) ? -1.0 : 1.0) * 3.0;
            tester_state.joint_torques(1, i) = -2.0;
            tester_state.joint_torques(2, i) = 7.5;
        }
...
int main(void) {
    ...
    LoopFunc loop_torque("torque_loop", DT_CTRL, boost::bind(&runTorque));
