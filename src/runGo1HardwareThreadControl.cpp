...
#include <thread>         
#include <chrono>        
#include <atomic>      // probably will need later
...
public:
  ...
  bool runHardware = true;
  std::chrono::duration<double> udpMethodTimeStep{static_cast<double>(dt)};
  std::chrono::duration<double> robotControlTimeStep{static_cast<double>(mpcInterval)};
...
void Custom::RobotControl() {
  ...
  while (runHardware) {
    ...
    //                         hardware_go1_state.joint_torques.block<3, 1>(0, 1), // FL
...
int main(void) {
  ...
  std::thread udpGet(&Custom::UDPRecv, &custom);
  std::thread udpSend(&Custom::UDPSend, &custom);
  std::thread robotControl(&Custom::RobotControl, &custom);

  std::cout << "Press Enter to stop" << std::endl;
  std::cin.get();       // pause the main method until enter key is pressed


  custom.runHardware = false;
  if (udpGet.joinable()) {
    udpGet.join();
  }
  if (udpSend.joinable()) {
